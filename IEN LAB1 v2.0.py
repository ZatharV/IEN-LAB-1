...
def createGraph(num, start_num):
        ...
        for i in range(3):
            for j in range(3):
                ...
                for m in graph[mat[i][j]]:
                    if m in neighbour:
                        neighbour.remove(m)
                # print(mat[i][j], "\t", neighbour, "\t", len(neighbour), end = "\t")
    ...
    # nt.add_node(ord(cornerNode), label=cornerNode, font='30px', x= 900 , y=300)
    for i in range(3):
        for j in range(3):
            if start_num == 65:
                shift = 0
...
# print(listOfConnectedNodes)
print("Created Network is shown")
...
while True:
    ...
    valid = False
    while not valid:
        startNode = input("Enter start Node: ").upper()
        if startNode == "END":
            end = True
            break
    ...
    valid = False
    while not valid:
        endNode = input("Enter End Node: ").upper()
        if endNode == "END":
            end = True
            break
    ...
    # shortestPath = bellmanFord(startNode,listOfConnectedNodes,numberOfNodes)
    # # shortest path is a list of list
